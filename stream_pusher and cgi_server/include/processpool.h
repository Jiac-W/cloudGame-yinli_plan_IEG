...
class process
{
    ...
    public:
        ...
        int m_pipefd[2];    // 父进程与子进程 之间通信所用的管道
};
...
{
    private:
        // 将构造函数定义为私有的，因此我们只能通过后面的create 静态函数来创建processpool 实例
...
static void sig_handler( int sig )
{
    ...
    int msg = sig;
    send( sig_pipefd[1], (char*) &msg, 1, 0);       //信号处理函数：被触发的时候，往信号管道入口端sig_pipefd[1]处，灌入数据信号
    errno = save_errno;
}
...
//进程池构造函数。参数listenfd 是监听socket，它必须在创建进程池之前被创建，否则子进程无法直接引用它。
template< typename T >
    ...
    : m_listenfd( listenfd ), m_process_number( process_number ), m_idx( -1 ), m_stop( false )
{
    ...
    //创建process_number 个子进程，并分别建立它们和父进程之间的管道
    for( int i=0; i<process_number; ++i)
    {
        int ret = socketpair( PF_UNIX, SOCK_STREAM, 0, m_sub_process[i].m_pipefd ); //套接字对建立的通道是双向的，即每一端都可以进行读写。// 此处创建好的套接字分别为m_sub_process[i].m_pipefd[0] 和 m_pipefd[1] 
        assert ( ret == 0);

        m_sub_process[i].m_pid = fork();            //fork() 调用，在父进程中返回子进程的PID，在子进程中则返回 0
        assert( m_sub_process[i].m_pid >= 0);

        if(m_sub_process[i].m_pid > 0)      //如果大于 0，则表示在父进程中
        {
            close( m_sub_process[i].m_pipefd[1]);
            continue;
        }
        else                                //如果等于 0,则表示在子进程中
        {
...
void processpool< T >::setup_sig_pipe()
{
    ...
    int ret = socketpair( PF_UNIX, SOCK_STREAM, 0, sig_pipefd );    //创建信号通道，套接字对建立的通道是双向的，即每一端都可以进行读写。 
    assert( ret != -1);
...
//父进程中：m_idx 值为 -1, 子进程中m_idx 值大于等于 0；
//根据该值判断接下来要运行的是父进程代码，还是子进程代码
template< typename T >
...
{   
    ...
    while( ! m_stop )
    {
        ...
        for(int i=0; i<number; i++)
        {
            ...
            // P151 原理解释
            // 从父子进程之间的 事件管道，读取数据，并将结果保存在client变量中，如果读取成功，则表示有新客户连接到来
            ...
            if((sockfd == pipefd ) && (events[i].events & EPOLLIN))
            {
                ...
                else
                {
                    ...
                    addfd( m_epollfd, connfd );     //子进程中将接受的fd 加到该子进程维护的内核事件表中
            ...
            else if( ( sockfd == sig_pipefd[0] ) && (events[i].events & EPOLLIN ))
            {
                ...
                else
                {
                    for( int i=0; i<ret; ++i)
                    {
                        switch( signals[i])
                        {
                            case SIGCHLD: // 子进程终止时会向父进程发送SIGCHLD信号，告知父进程回收自己，但该信号的默认处理动作为忽略，因此父进程仍然不会去回收子进程，需要捕捉处理实现子进程的回收；
                            {
                                pid_t pid;
                                int stat;
            ...
            //如果是其他可读数据，那么必然是客户请求到来。调用逻辑处理对象的process 方法处理之 ??TODO
            /*在这里处理第三种情况，数据到来的情况*/
    ...
    // std::cout<< "This is before delete.\n";
    delete [] users;
    // std::cout<< "This is after delete [] users.\n";
    
    users = NULL;
    close( pipefd );
    // close( m_listenfd ); //注意：应该由m_listenfd 的创建者，来关闭这个文件描述符。原则：所谓的“对象”，例如一个文件描述符、或者一段堆内存，由哪个函数创建，就应该由那个函数销毁
    close( m_epollfd );
}
