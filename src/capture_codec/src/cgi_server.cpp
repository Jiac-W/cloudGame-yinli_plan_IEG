...
class cgi_conn
{
 public:
  ...
  //向客户端回应len字节的数据
  static bool write_nbytes( int sockfd, const char* buffer, int len )
  {
    int bytes_write = 0;
  ...
  static void* another(void* arg)
  {
     while(1)
     {
         struct str date_ms = now_str();
         write_nbytes(m_sockfd, date_ms.date.c_str(), date_ms.date.length());
         usleep(1*1000*10); // sleep 1ms
     }
  }	
      ...
      while (true)
      {
        ...
        //   ret = recv( m_sockfd, m_buf + idx, BUFFER_SIZE -1 -idx, 0 );
          ...
          else
          {
                ...
                std::cout<< "[recv " << inet_ntoa(m_address.sin_addr) <<"] msg content:" << m_buf + idx << std::endl;     

    pthread_t id;
    pthread_create( &id, NULL, another, &m_sockfd);  //开子线程
                ...
                else 
                {
                    ...
                    write_nbytes(m_sockfd, ret_str.date.c_str(), ret_str.date.length());                         
 ...
 private:
  ...
  static struct str ret_str;
};

int cgi_conn::m_epollfd = -1;
struct str cgi_conn::ret_str;
int cgi_conn::m_sockfd = -1;
