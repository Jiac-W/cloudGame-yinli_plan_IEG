    ...
    : m_listenfd( listenfd ), m_process_number( process_number ), m_idx( -1 ), m_stop( false )
{
    ...
    {
        int ret = socketpair( PF_UNIX, SOCK_STREAM, 0, m_sub_process[i].m_pipefd ); //套接字对建立的通道是双向的，即每一端都可以进行读写。 
        assert ( ret == 0);
...
void processpool< T >::setup_sig_pipe()
{
    ...
    setnonblocking( sig_pipefd[1]);     //信号管道 入口端设置为非堵塞
...
{   
    ...
    //每个子进程都通过其在进程池中的序号m_idx 找到与父进程通信的管道m_pipefd
        ...
        for(int i=0; i<number; i++)
        {
            ...
            if((sockfd == pipefd ) && (events[i].events & EPOLLIN))
            {
                ...
                else
                {
                    ...
                    addfd( m_epollfd, connfd );     //子进程中将接受的fd 加到改子进程维护的内核事件表中
